...
from interview_system.orchestration.graph import get_interview_workflow
...
async def main():
    ...
    while True:
        ...
        current_question_val = current_state.values.get("current_question")
        if not current_question_val:
            break
        
        if isinstance(current_question_val, dict):
            current_question_obj = QuestionTurn(**current_question_val)
        else:
            current_question_obj = current_question_val
        ...
        payload = {
            ...
            "current_rubric": {}, # Pass a dummy rubric
        }
        ...
        async for chunk in graph.astream(None, config=config, at="run_evaluation"):
            ...
            if node_name == "fast_evaluator":
                ...
                if node_state and node_state.get("current_question"):
                    cq = node_state["current_question"]
                    evals = cq.get("evals", {})
                    fast_eval = evals.get("fast_eval", {})
        ...
        # --- THIS IS THE FIX ---
        # The history list contains QuestionTurn objects, but the 
        # 'feedback' attribute on them is a DICTIONARY.
        if updated_state.values.get("question_history"):
            last_turn = updated_state.values["question_history"][-1]
            
            # feedback is a dict, so we must use .get()
            ...
            if feedback:
                ...
                if improvement_points:
                     # point is also a dict
                     for point in improvement_points:
                        # Use .get() for dictionary access
                        print(f"   - {point.get('bullet')}")
                else:
    ...
    async for chunk in graph.astream(None, config=config, at="final_reporting_entry"):
        pretty_print_chunk(chunk)
